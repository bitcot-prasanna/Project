...
- (void)setVideoVolumn:(float)volumn{
    self.volumn = volumn;
    [self.avPlayer setVolume: self.volumn];
}
...
- (void)playerDidPlayToEnd:(NSNotification *)note
{
    ...
    [self pauseContent];
    ll_run_on_ui_thread(^{
        [self seekToTimeInSecond:0.0 userAction:true completionHandler:^(BOOL finished) {
...
{
    return [self.avPlayer ll_currentItemDuration];
}
- (BOOL)stalling
{
...
- (BOOL)isPlayingVideo
{
    if (self.avPlayer == nil ) { return false; }else{
